...
# Storyboard image prompts are generated ONE PER ACT (3 total). Each act's
# prompt is the shared style preamble + that act's four-panel outline. Keep
# in sync with server/prompts.go (storyboardImgPreamble / storyboardAct*Panels).
_SB_PREAMBLE = """You are an expert AI instructional designer and technical illustrator. Create a detailed infographic storyboard for ONE act of a 3-act explainer video, using a sequential four-panel comic strip format to visually explain a technical question and answer regarding system/LLM architecture.
...
_SB_ACT1 = """#### Act 1: The Agents Report
...
_SB_ACT2 = """#### Act 2: The Coordinator's Dilemma
...
_SB_ACT3 = """#### Act 3: The Synthesis Solution
...
def _storyboard_act_prompts() -> dict:
    return {
        "act-1": _SB_PREAMBLE + _SB_ACT1,
        "act-2": _SB_PREAMBLE + _SB_ACT2,
        "act-3": _SB_PREAMBLE + _SB_ACT3,
    }
...
_DEFAULTS = {
    ...
    "storyboard": {
        ...
        "act_prompts": _storyboard_act_prompts(),
    },
}
...
def storyboard():
    p = load("storyboard")
    ap = p.get("act_prompts") or {}
    # Fall back to compiled defaults for any missing or empty act.
    for k, v in _storyboard_act_prompts().items():
        if k not in ap or not (ap[k] or "").strip():
            ap[k] = v
    return p.get("system", ""), p.get("user", ""), ap
